...
The FM-index \citep{baeza_yates2009} is a compressed full-text substring index built on the Burrows--Wheeler Transform (BWT) of the concatenated reference collection. Bit-Pop constructs the FM-index using an exact suffix array (SA) computed via radix sort, which avoids the approximation errors of SA-IS while maintaining linear-time construction.
...
\begin{enumerate}
    \item Extracts all overlapping k-mers (default $k=8$) from the read.
...
\addplot[fill=blue!30] coordinates {(k=15,60000) (k=12,35000) (k=10,4800)};
\legend{Throughput}
\end{axis}
\end{tikzpicture}
...
Source code is available at \url{https://github.com/pop/bit-pop} under the MIT License. Benchmark scripts and example datasets are included in the repository. A Zenodo archive with citable software artifact will be published upon acceptance.
