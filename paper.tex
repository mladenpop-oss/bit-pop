...
\item \textbf{Performance trade-off with top-$N$:} Using top-$N$ rarest k-mers (e.g., $N=3$) improves mapping rate by ~1.4\% but increases computation time by ~3x compared to single-anchor mode. The recommended setting ($N=2$) provides a balance between speed and accuracy.
